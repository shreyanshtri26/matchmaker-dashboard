...
// Define and export the getMatches function for routes
export const getMatches = async (req: Request, res: Response) => {
  try {
    ...
    // Get opposite gender and only dummy client pool (for matchmaking)
    const oppositeGender = customer.gender === 'Male' ? 'Female' : 'Male';

    // Fetch all dummy profiles of opposite gender
    const potentialMatches = await CustomerModel.find({
      gender: oppositeGender,
      isDummy: true
    });

    // Generate matches with scores and intros
